...
- **能隐式就别显式**:`AnimatedFoo`/`TweenAnimationBuilder` 优先,显式仅在需精确控制时。
- **动画是状态驱动**:由 controller/state 推动重建,别命令式改属性。
- **动效有性能预算**:重动画用 `RepaintBoundary` 隔离,盯 raster 线程(见 mindset 模型 4)。

**诚实边界:**

- “好看/舒服”是设计判断,本 skill 只保证技术正确与性能可控。
- 三方动效(Lottie/Rive)性能依素材复杂度,需真机实测。
