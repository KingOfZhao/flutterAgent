...
- **CI 是不可绕过的门禁**:format→analyze→test→build 全绿才允许合并。
- **可重复 > 手工**:发布走管线与 flavors,不靠“在某台机器上手动打”。
- **release 必留符号**:`--obfuscate --split-debug-info`,否则线上栈无法还原。

**诚实边界:**

- 具体 CI 平台(GH Actions/Codemagic/fastlane)语法各异,以其文档为准。
- 商店审核与签名密钥涉及账号权限,需人工合规处理。
